...
Section Termes.
  ...
  Inductive term : Set :=
    ...
    | App : term -> term -> term
    | Prod : term -> term -> term
    | Elim : term -> term -> term -> term -> term.

  Fixpoint lift_rec (n : nat) (t : term) (k : nat) {struct t} : term :=
    match t with
    | Srt s => Srt s
    | Zero => Zero
    | Succ m => Succ (lift_rec n m k)
    | Add p q => Add (lift_rec n p k) (lift_rec n q k)
    | Ref i =>
    ...
    | Elim Q f g m => 
        Elim (lift_rec n Q k) (lift_rec n f k) (lift_rec n g k) (lift_rec n m k)
    end.
  ...
  Fixpoint subst_rec (N M : term) (k : nat) {struct M} : term :=
    ...
    | Srt s => Srt s
    | Zero => Zero
    | Succ t => Succ (subst_rec N t k)
    | Add p q => Add (subst_rec N p k) (subst_rec N q k)
    | Ref i =>
    ...
    | Elim Q f g m => 
        Elim (subst_rec N Q k) (subst_rec N f k) (subst_rec N g k) (subst_rec N m k)
    end.
  ...
  Inductive subterm : term -> term -> Prop :=
    | sbtrm_succ : forall A, subterm A (Succ A)
    | sbtrm_add_l : forall A B, subterm A (Add A B)
    | sbtrm_add_r : forall A B, subterm B (Add A B)
    ...
    | sbtrm_elim_Q : forall Q f g n, subterm Q (Elim Q f g n)
    | sbtrm_elim_f : forall Q f g n, subterm f (Elim Q f g n)
    | sbtrm_elim_g : forall Q f g n, subterm g (Elim Q f g n)
    | sbtrm_elim_n : forall Q f g n, subterm n (Elim Q f g n).
  ...
  Inductive foterm : term -> Prop := 
    | Val : forall n, foterm (Ref n)
    | Cst_0 : foterm Zero
    | Cst_S : forall m, foterm m -> foterm (Succ m)
    | Df_Add : forall m n, foterm m -> foterm n -> foterm (Add m n).

  Definition substitution := list term.

  Definition app_subst M s := fold_left (fun m n => subst n m) s M.
