...
Require Import ZF ZFpairs ZFcoc ZFlambdaF.
Import IZF. 
...
split; intros.
 ...
 revert k H; induction t; simpl; intros.
  destruct (lt_eq_lt_dec k n) as [[?|?]|?]; simpl in H.
   destruct (le_gt_dec k (pred n)); simpl in *.
    elim H.
...
| Nf_app : forall u v,
    match u, v with
    | Lc.Abs _, _ | Lc.Fix _, Lc.Abs _ => False
    | _,_ => True end -> nf u -> nf v -> nf (Lc.App u v)
| Nf_abs : forall t, nf t -> nf (Lc.Abs t)
| Nf_fx : forall t, nf t -> nf (Lc.Fix t).
...
destruct t.
 ...
 destruct (red1_dec t1).
  destruct s.
  left; exists (Lc.App x t2); auto with coc.

  destruct (red1_dec t2).
   destruct s.
   ...
   destruct t1;[right;simpl;auto|left|right;simpl;auto|].
    ...
    destruct t2;[right; simpl; auto|left|right;simpl;auto|right;simpl;auto].
    exists (Lc.App (Lc.subst (Lc.Fix t1) t1) (Lc.Abs t2)); auto with coc.

 destruct (red1_dec t).
  destruct s.
  left; exists (Fix x); auto with coc.
