...
Section Terms.
  ...
  Fixpoint lift_rec (n : nat) (t : term) (k : nat) {struct t} : term :=
    ...
    | Int _ => t
    | Ifn c f => Ifn (lift_rec n c k) (lift_rec n f k)
    end.
  ...
  Fixpoint subst_rec (N M : term) (k : nat) {struct M} : term :=
    ...
    | Int _ => M
    | Ifn c f => Ifn (subst_rec N c k) (subst_rec N f k)
    end.
  ...
  Inductive subterm : term -> term -> Prop :=
    ...
    | sbtrm_ifn_l : forall A B, subterm A (Ifn A B)
    | sbtrm_ifn_r : forall A B, subterm B (Ifn A B).
...
Section Beta_Reduction.
  ...
  Inductive red1 : term -> term -> Prop :=
    ...
    | iota : forall n f, red1 (Ifn (Int n) f) f
    ...
    | ifn_red_l :
        forall M1 N1, red1 M1 N1 -> forall M2, red1 (Ifn M1 M2) (Ifn N1 M2)
    | ifn_red_r :
        forall M2 N2, red1 M2 N2 -> forall M1, red1 (Ifn M1 M2) (Ifn M1 N2).
...
Hint Resolve red_red_app : coc.
  ...
  Instance ifn_red_morph : Proper (red ==> red ==> red) Ifn.
do 3 red; induction 1; intros.
 induction H; intros; auto with coc.
 transitivity (Ifn x P); auto with coc.

 transitivity (Ifn P y); auto with coc.
Qed.

  Lemma red_red_ifn :
    forall u u0, red u u0 ->
    forall v v0, red v v0 ->
    red (Ifn u v) (Ifn u0 v0).
Proof ifn_red_morph.
Hint Resolve red_red_ifn : coc.
...
induction 1; intros.
 ...
 destruct t'; simpl in *; try discriminate.
  destruct (le_gt_dec k n1); discriminate H.

  destruct t'1; try discriminate H.
   simpl in H.
   destruct (le_gt_dec k n1); discriminate H.

   exists t'2; auto with *.
   injection H; auto.
...
exists (Ifn z B); auto with coc.

exists (Ifn A z); auto with coc.
Qed.
