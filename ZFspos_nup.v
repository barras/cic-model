...
constructor; simpl; intros; auto.
 ...
 apply ZFspos.iso_reccall; trivial.
Qed.
...
*)
Lemma iso_trad_sum F G X :
  morph1 X ->
  isPositive F ->
  isPositive G ->
  iso_fun (sum (dp_oper F X) (dp_oper G X))
     (W0.W_Fd (fun _ => sum (w1 F) (w1 G))
        (fun _ => sum_case (w2 F) (w2 G))
        (fun _ x i => sum_case (fun x : set => w3 F x i)
                               (fun x : set => w3 G x i) x) X empty)
     (ZFspos.trad_sum (@dp_iso F) (@dp_iso G)).
intros Xm Fp Gp.
unfold W0.W_Fd, trad_sum.
eapply iso_fun_trans.
 apply sum_iso_fun_morph;[apply Fp|apply Gp]; trivial.

 eapply iso_change_rhs.
 2:apply iso_fun_sum_sigma; auto.
 apply sigma_ext; auto with *.
 intros.
 rewrite cc_prod_sum_case_commut with (3:=H).
  apply cc_prod_ext.
   apply sum_case_morph; auto with *.
    apply ww2m; trivial; reflexivity.
    apply ww2m; trivial; reflexivity.
  red; intros.
...
Lemma iso_prodcart X1 X2 A1 A2 B1 B2 f1 f2 Y f g :
   morph1 Y ->
   morph1 B1 ->
   morph1 B2 ->
   morph2 f1 ->
   morph2 f2 ->
   iso_fun X1 (W0.W_Fd (fun _ => A1) (fun _ => B1) (fun _ => f1) Y empty) f ->
   iso_fun X2 (W0.W_Fd (fun _ => A2) (fun _ => B2) (fun _ => f2) Y empty) g ->
   iso_fun (prodcart X1 X2)
     (W0.W_Fd (fun _ => prodcart A1 A2)
     (fun _ x => sum (B1 (fst x)) (B2 (snd x)))
     (fun _ x => sum_case (f1 (fst x)) (f2 (snd x))) Y empty)
     (trad_prodcart B1 B2 f g).
intros Ym B1m B2m f1m f2m isof isog.
unfold W0.W_Fd, ZFspos.trad_prodcart.
eapply iso_fun_trans.
 apply prodcart_iso_fun_morph; [apply isof|apply isog].
assert (m1 : ext_fun A1 (fun x => cc_prod (B1 x) (fun i => Y (f1 x i)))).
 do 2 red; intros.
 apply cc_prod_ext; auto with *.
 red; intros.
 apply Ym.
 apply f1m; auto with *.
assert (m1' : ext_fun A2 (fun x => cc_prod (B2 x) (fun i => Y (f2 x i)))).
 do 2 red; intros.
 apply cc_prod_ext; auto with *.
 red; intros.
 apply Ym.
...
eapply iso_fun_trans.
 ...
 apply sigma_iso_fun_morph; auto.
  ...
  apply cc_prod_ext; auto.
  red; intros.
  apply sum_case_ind0 with (2:=H1); auto with *.
   do 2 red; intros.
   rewrite H3; reflexivity.

   intros.
   rewrite H4 in H2; symmetry in H2.
   rewrite sum_case_inl0; eauto.
   rewrite H2,H0,H4; reflexivity.
   ...
   do 2 red; intros.
   apply Ym; apply f1m; auto with *.

   do 2 red; intros.
   apply Ym; apply f2m; auto with *.
Qed.
...
Lemma iso_arg_norec P X A B f Y h :
  morph1 Y ->
  morph1 X -> 
  morph1 A ->
  morph2 B ->
  ...
  morph2 h ->
  (forall x, x ∈ P ->
   iso_fun (X x) (W0.W_Fd (fun _ => A x) (fun _ => B x) (fun _ => f x) Y empty) (h x)) ->
  iso_fun (sigma P X)
   (W0.W_Fd (fun _ => sigma P A)
            (fun _ x => B (fst x) (snd x))
            (fun _ x => f (fst x) (snd x)) Y empty)
   (trad_sigma h).
intros Ym Xm Am Bm fm hm isoh.
unfold W0.W_Fd, ZFspos.trad_sigma.
eapply iso_fun_trans.
 apply sigma_iso_fun_morph with (4:=id_iso_fun _)
  (B':=fun x => W0.W_Fd (fun _ => A x) (fun _ => B x) (fun _ => f x) Y empty); auto.
  do 2 red; intros.
   unfold W0.W_Fd.
  ...
  intros.
  rewrite H0 in H.
  specialize isoh with (1:=H).
  revert isoh; apply iso_fun_morph; auto with *.
...
Lemma iso_param P X A B f Y h :
  ...
  (forall x, x ∈ P ->
   iso_fun (X x) (W0.W_Fd (fun _ => A x) (fun _ => B x) (fun _ => f x) Y empty) (h x)) ->
  iso_fun (cc_prod P X)
   (W0.W_Fd (fun _ => cc_prod P A)
            (fun _ x => Σ z ∈ P, B z (cc_app x z))
            (fun _ x i => f (fst i) (cc_app x (fst i)) (snd i)) Y empty)
   (trad_cc_prod P B h).
intros Ym Xext Am Bm fm hm isoh.
unfold W0.W_Fd, ZFspos.trad_cc_prod.
eapply iso_fun_trans.
 apply cc_prod_iso_fun_morph with (4:=id_iso_fun _)
  (B':=fun x => W0.W_Fd (fun _ => A x) (fun _ => B x) (fun _ => f x) Y empty); trivial.
  do 2 red; intros.
   unfold W0.W_Fd.
   apply sigma_morph; auto with *.
   red; intros.
...
split; repeat red; simpl; intros.
 ...
 assert (tmp1 :
   cc_prod_isomap x (fun x3 => x3) (fun y3 => dp_iso (x0 y3)) x1 ==
   cc_prod_isomap y (fun x3 => x3) (fun y3 => dp_iso (y0 y3)) y1).
  unfold cc_prod_isomap.
  apply cc_lam_morph; auto with *.
  red; intros.
  assert (eqf := H0 _ _ H2).
  apply eq_diso; trivial.
  apply cc_app_morph; trivial.
 assert (tmp2 :
     cc_prod_sigma_iso x
        (cc_prod_isomap x (fun x2 : set => x2)
           (fun y2 : set => dp_iso (x0 y2)) x1) ==
     cc_prod_sigma_iso y
        (cc_prod_isomap y (fun x2 : set => x2)
           (fun y2 : set => dp_iso (y0 y2)) y1)).
...
Instance dINDi_morph_gen : Proper (eq_set==>(eq_set==>eqdpos)==>eq_set==>eq_set==>eq_set) dINDi.
do 5 red; intros.
unfold dINDi.
...
Instance dIND_morph_gen : Proper (eq_set==>(eq_set==>eqdpos)==>eq_set==>eq_set) dIND.
do 4 red; intros.
unfold dIND.
