...
intros n T getn.
assert (val_ok e (V.shift (length l) i)).
 apply typ_sub_shift' in H0.
 replace (length l) with (length (sub_rev_ctxt s l)); trivial.
 clear.
 induction l; simpl; auto.
specialize H with (1:=H1).
unfold V.lams at 2; simpl.
destruct (le_gt_dec (length l) n).
 assert (nth_error f (n - length l) = Some T).
  unfold value in getn.
  rewrite <- getn.
  clear -l0; revert n l0; induction l; simpl; intros.
   rewrite <- Minus.minus_n_O; trivial.

   destruct n.
    inversion l0.
    apply IHl; auto with *.
 generalize (H (n - length l) _ H2).
 intros.
 ...
 destruct (le_gt_dec (length l) (S (n+a))).
  destruct (length l).
  rewrite <- Minus.minus_n_O; reflexivity.
  replace (S (n - S n0 + a)) with (n+a-n0); [reflexivity|omega].
 exfalso; omega.

 red in H0.
 apply nth_sub_rev' with (f:=e)(s:=s) in getn; trivial.
 specialize H0 with (1:=getn).
 destruct T as [(T,Tm)|]; simpl in *; trivial.
 red in H0|-*.
 revert H0; apply eq_elim.
 apply Tm.
 ...
 red.
 unfold V.lams, V.shift; simpl.
 destruct (le_gt_dec (length l - S n) a); destruct (le_gt_dec (length l) (S (n+a)));
  try (exfalso;omega); auto with *.
 destruct (length l); simpl.
  inversion g.
 replace (a-(n0-n)) with (n+a-n0) by omega.
 apply sub_m.
 intros a'.
 red. 
 replace (S (n + (n0-n+a'))) with (S (n0+a')) by omega.
 reflexivity.
Qed.
