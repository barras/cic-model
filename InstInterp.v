...
Lemma intp_hyp_nth_term : forall hyp n, 
...
induction f; simpl intp_fofml; intros.
 ...
 apply Prod_morph; [apply Prod_morph; simpl; (split; [red|]); reflexivity|];
  repeat rewrite eq_term_lift_ref_bd by omega; repeat rewrite <- lift_intp_lift_term_rec;
...
induction t; intros; simpl intp_foterm.
 simpl; destruct (lt_eq_lt_dec k n) as [[lt|eq]|bt].
...
Lemma subst_intp_subst_term : forall t nn, 
  eq_term (subst (intp_foterm nn) (intp_foterm t)) 
         (intp_foterm (subst_term t nn)).
unfold subst. intros. apply subst_intp_subst_term_rec with (k:=0).
Qed.
 
Lemma subst_intp_subst_fml_rec : forall f nn k,
  eq_term (subst_rec (intp_foterm nn) k (intp_fofml f)) 
         (intp_fofml (subst_fml_rec f nn k)).
induction f; simpl intp_fofml; intros.
...
Lemma subst_intp_subst_fml : forall f nn,
  eq_term (subst (intp_foterm nn) (intp_fofml f)) 
          (intp_fofml (subst_fml  f nn)).
unfold subst; intros; apply subst_intp_subst_fml_rec with (k:=0).
Qed.

Lemma hyp_ok_add : forall t1 t2 hyp,
  hyp_ok_term hyp t1 /\ hyp_ok_term hyp t2 <-> hyp_ok_term hyp (Df_Add t1 t2).
split; intros.
 destruct H as (H1, H2). 
 red in H1, H2 |- *. simpl in H1, H2 |- *; intros.
 unfold fv_term in H; simpl in H; apply in_app_or in H.
 destruct H as [Hl|Hr]; [apply H1 in Hl|apply H2 in Hr]; trivial.

 split; red in H |- *; intros; simpl in H, H0.
  assert (In n (fv_term_rec t1 0 ++ fv_term_rec t2 0)).
...
induction t; simpl intp_foterm; intros.
 ...
 apply intp_hyp_nth_term in H. apply H0 in H; clear H0.
